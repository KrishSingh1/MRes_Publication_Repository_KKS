...
growth.form <- read.csv('DATASETS/AusPlots_VegType_PC_Height_Rule.csv')
site_visits <- unique(one_point[,c('site_unique', 'visit_start_date')])
...
site_info <- readRDS('DATASETS/site_veg_Final2-0-6.rds')
site_date <- site_info$site.info %>%
  select(site_unique, site_location_name, visit_start_date) %>%
  subset(site_location_name %in% unique(one_point$site_location_name))
...
plot_grid(plotlist = list_plots_examples, ncol = 3, scale = 1.2)
