...
site_info <- read.csv('../DATASETS/AusPlots_Extracted_Data/Final/DEA_FC_Ground_Truth_Evaluation_complete.csv') %>%
  subset(fraction_type == 'green') %>% select(!other) %>% na.omit()

revisited <- names(which(table(site_info$site_location_name) > 1))
...
# SAABHC0004
## Some other examples:
# NSABHC0009 --> Phenological dates align, still short-term slopes are steeper, but do not go the opposite direction
...
ecdf_green <- ggplot(data = dea_fc) +
  ...
  scale_colour_manual(values= brewer.pal(n = number_of_visits + 1, "Oranges")[2:(number_of_visits+1)])
plot(ecdf_green)
...
site_info <- read.csv('../DATASETS/AusPlots_Extracted_Data/Final/DEA_FC_Ground_Truth_Evaluation_complete.csv') %>%
  subset(fraction_type == 'green') %>% select(!other) %>% na.omit()
revisited <- sort(names(which(table(site_info$site_location_name) > 1)))

doc <- read_docx() 
plot_file <- tempfile(fileext = ".png")

pb <- txtProgressBar(min = 1, max = length(revisited), style = 3)
counter = 1
for(example_site in revisited) {
  ...
  number_of_visits <- nrow(evaluation.data.2)
  ecdf_green <- ggplot(data = dea_fc) +
    stat_ecdf(mapping = aes(x = pv_filter), color = 'darkgreen') +
    geom_point(data = evaluation.data.2,
               mapping = aes(x = pv_filter, y = pv_filter_percentile, color = time),
  ...
  ggsave(plot_file, combined_plot, width = 6, height = 8, units = "in")
  doc <- doc %>% 
    body_add_par(value = example_site, style = "heading 1") %>%
    body_add_img(src = plot_file, width = 6, height = 8, style = "centered") %>%
    body_add_break()
  
  counter <- counter + 1
}
print(doc, target = paste0("../STEP4_EXPORE_DATA/AusPlots_Thiel-Sens_Plots.docx"))
