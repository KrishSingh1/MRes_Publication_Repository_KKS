...
trim_to_nearest_coord <- function(site.corners.data, dea.fc.i, query, buffer = 30, plot_result = FALSE) {
  ...
  # Subset the site corners data by the query 
    ...
    print(paste0('Buffer to ', buffer, ' and trim!'))
    trimmed <- dea.fc.i %>% 
...
directory <- 'E:/DYNAMIC MODEL VEGETATION PROJECT/au_dyanamic_vegetation_project/DATASETS/DEA_FC_PROCESSED/RawDataCurrent/NewBatchCurrent'
files <- list.files(directory, pattern = "\\.csv$", full.names = FALSE)
file.names <- tools::file_path_sans_ext(files)

site.corners.data <- read.csv('E:/DYNAMIC MODEL VEGETATION PROJECT/au_dyanamic_vegetation_project/DATASETS/AusPlots_Location/AusPlots_Published_Corner_Points_20240701/Published Plot Corners_extract26062024_cleaned.csv')
